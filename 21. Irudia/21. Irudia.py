...
for k in range (0,3):
    ...
    q_dot[0]= (qt[1] - qt[0]) / delta_t
    r_dot[0]= (rt[1] - rt[0]) / delta_t
    ...
    for i in range (0,step-1):
        ...
        q_dot[i+1]= (qt[i+1] - qt[i]) / delta_t
        r_dot[i+1]= (rt[i+1] - rt[i]) / delta_t
    if k==0:
...
ax2.plot(Vt_1, V_dot_1, 'g', label=r'$i_{e}=6.0$')
...
ax2.legend(bbox_transform=ax2.transData, bbox_to_anchor=(8, 195), ncol=1, borderaxespad=0, 
             frameon=False, fontsize=15)

fig.text(10, 220.0, r'\textrm{Mintz-Potentziala Fase-espazioan}', size=25, va='center', transform=ax2.transData);


ax3 = plt.subplot(222)
ax3.set_xlim(0,20)
ax3.set_ylim(130,210)
ax3.plot(Vt_2, V_dot_2, 'r',label=r'$i_{e}=7.0$')
...
ax3.legend(bbox_transform=ax3.transData, bbox_to_anchor=(8, 195), ncol=1, borderaxespad=0, 
             frameon=False, fontsize=15)
...
plt.savefig('IB_Eredua_Fase_Espazioa_2.pdf', format='pdf', dpi=180,bbox_inches="tight")
